...
@interface CJVerticalCenterTextView () {
    
}

@end

@implementation CJVerticalCenterTextView
...
//    self.placeholderView.backgroundColor = [UIColor colorWithWhite:0.2 alpha:0.2];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if ([keyPath isEqualToString:@"contentSize"] || [keyPath isEqualToString:@"frame"]) {
        [self cjTextView_adjustedContentInsetToTextCenter:0];
    }
}
 ...
 */
- (void)cjTextView_adjustedContentInsetToTextCenter:(CGFloat)delay {
    [self cj_adjustedContentInsetToTextCenter:delay];
    [self.placeholderView cj_adjustedContentInsetToTextCenter:delay];
    
//    self.placeholderViewTopLayoutConstraint.constant = 50;
}

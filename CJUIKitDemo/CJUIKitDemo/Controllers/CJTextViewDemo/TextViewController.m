...
//    _textView.placeholderColor = [UIColor redColor];
    ...
    [self.textView setMaxNumberOfLines:4 textHeightChangeBlock:^(NSString *text, CGFloat textHeight) {
        // 文本框文字高度改变会自动执行这个【block】，可以在这【修改底部View的高度】
        // 设置底部条的高度 = 文字高度 + textView距离上下间距约束
        // 为什么添加10 ？（10 = 底部View距离上（5）底部View距离下（5）间距总和）
        _bottomViewHeightConstraint.constant = textHeight + 10;
    }];
}
...
- (void)keyboardWillChangeFrame:(NSNotification *)note
{
    ...
    // 修改底部视图距离底部的间距
    _bottomViewBottomConstranit.constant = endFrame.origin.y != screenH?endFrame.size.height:0;

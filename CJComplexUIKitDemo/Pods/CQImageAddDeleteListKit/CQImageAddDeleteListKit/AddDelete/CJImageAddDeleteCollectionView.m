...
@property (nonatomic, copy) void(^addHandle)(CJImageAddDeleteCollectionView *bCollectionView);
...
- (instancetype)initWithConfigItemCellBlock:(void (^)(CJUploadCollectionViewCell *bItemCell, id bDataModel))configItemCellBlock
                            clickItemHandle:(void(^)(NSArray *bDataModels, NSInteger currentClickItemIndex))clickItemHandle
                                  addHandle:(void(^)(CJImageAddDeleteCollectionView *bCollectionView))addHandle
...
- (void)setupViews {
    ...
    /* 创建DataSource */
    CJImageAddDeleteCollectionViewDataSource *cjDataSource = [[CJImageAddDeleteCollectionViewDataSource alloc] initWithMaxShowCount:5 cellForPrefixBlock:nil cellForSuffixBlock:^UICollectionViewCell *(CJImageAddDeleteCollectionViewDataSource *bDataSource, UICollectionView *collectionView, NSIndexPath *indexPath) {
        ...
        cell.cjImageView.image = [UIImage imageNamed:@"CQImageAddDeleteListKit_AddDeleteBundle.bundle/cjCollectionViewCellAdd"];
        [cell.cjDeleteButton setImage:nil forState:UIControlStateNormal];
        
        return cell;
    } cellForItemBlock:^UICollectionViewCell *(CJImageAddDeleteCollectionViewDataSource *bDataSource, UICollectionView *collectionView, NSIndexPath *indexPath) {
...
- (void)addDtaModels:(NSArray *)dataModels {
    [self.imageModels addObjectsFromArray:dataModels];
    [self reloadData];
}
